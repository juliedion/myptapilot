...
const NAV_GROUPS: { label: string; items: NavItem[] }[] = [
  {
    ...
    items: [
      ...
      { id: 'ai', label: 'AI Assistant', icon: <Icon path="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />, gradient: 'gradient-vivid' },
    ],
  },
  {
    label: 'Communication',
    ...
    <aside className={`fixed left-0 top-0 h-full bg-white border-r border-slate-100 flex flex-col transition-all duration-300 z-30 shadow-sm ${isOpen ? 'w-64' : 'w-16'}`}>
      ...
      <div className="flex items-center gap-3 px-4 py-4 border-b border-slate-100">
        ...
        <button
          ...
          className="flex-shrink-0 w-7 h-7 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-400 hover:text-slate-600 transition-colors"
        >
                    ...
                    <li key={item.id}>
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium transition-all duration-150 ${
                          isActive
                            ? 'gradient-vivid text-white shadow-brand'
                      ...
                      >
                        ...
                        {isOpen && (
                          <>
                            ...
                            {item.badge && (
                              <span className={`text-xs rounded-full w-5 h-5 flex items-center justify-center flex-shrink-0 font-bold ${isActive ? 'bg-white/25 text-white' : 'bg-brand-100 text-brand-600'}`}>
                                {item.badge}
                              </span>
                            )}
                            {item.treasurerOnly && (
                              <svg className={`w-3 h-3 flex-shrink-0 ${isActive ? 'text-white/70' : 'text-amber-400'}`} fill="currentColor" viewBox="0 0 20 20">
          ...
          {isOpen && (
            <>
              <div className="flex-1 min-w-0">
                ...
                <span className={`badge text-[10px] ${roleColors[user?.role || 'member']}`}>
                  {user?.role?.charAt(0).toUpperCase()}{user?.role?.slice(1)}
                </span>
              </div>
              <button onClick={logout} className="text-slate-300 hover:text-red-400 transition-colors p-1" title="Sign out">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
